...
- 3D画面左下の `初期状態` ボタンで、見た目変更・最新シミュレーション結果・実装依頼ファイル・画面上の操作状態を既定値に戻す
...
- `maze_agent`: 壁色、床色、経路色、スタート色、ゴール色、エージェント色。壁色の既定値は灰色 (`#566273`)
...
- Codex実装待ちキューとwatcher出力
- 画面上のシミュレーション選択、入力パラメータ、再生速度、視点、チャット表示、Codex依頼案パネル

削除される実装依頼ファイルは次の通りです。

```text
logs/codex_tasks/implementation_requests.jsonl
logs/codex_tasks/events.jsonl
logs/codex_tasks/pending_implementation/
logs/codex_tasks/watcher_outputs/
logs/codex_tasks/watcher_state.json
logs/codex_tasks/watcher.lock  # watcher停止中または古いlockの場合
```

会話ログと保存済みCodex依頼案本文のログ `logs/codex_tasks/tasks.jsonl` は削除しません。
`events.jsonl` は、古い実装依頼状態を消したあと、今回のリセットイベントだけが新しく記録されます。
